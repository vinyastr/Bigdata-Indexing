...
//                return ResponseEntity.status(HttpStatus.BAD_REQUEST).eTag(planService.getEtags(planId)).body("PlanId not present");
            ...
            if(ex!=null)return new ResponseEntity(map,headers,status);
            else throw ex;
        }
           }
    ...
    public ResponseEntity<String> patchPlan1(@PathVariable(value = "id") String planId, @RequestBody String plan,@RequestHeader HttpHeaders headers) throws Exception {
        try {
            ...
            boolean success = planService.updatePlanPatch(token,planId,etag,plan);
            if(success){
                map.clear();
            ...
            headers1.add("ETag","\""+planService.getEtags(planId)+"\"");
            HttpStatus status = planService.getStatus(ex);
            if(ex!=null)return new ResponseEntity(map,headers1,status);
            else throw ex;
        }
    }
}
